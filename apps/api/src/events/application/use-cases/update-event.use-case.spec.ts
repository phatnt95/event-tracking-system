...
describe('UpdateEventUseCase', () => {
  ...
  const mockUpdatedEvent = new Event(
    mockEventId,
    mockBabyId,
    EventType.FEED,
  ...
  it('should update event successfully', async () => {
    ...
    eventRepo.findById.mockResolvedValue(mockExistingEvent);
    eventRepo.update.mockResolvedValue(mockUpdatedEvent);
    ...
    expect(eventRepo.update).toHaveBeenCalledWith(mockEventId, {
      occurredAt: new Date('2026-07-27T10:30:00.000Z'),
      note: 'Updated note',
    });
    expect(result.note).toBe('Updated note');
    expect(result.occurredAt).toBe('2026-07-27T10:30:00.000Z');
  });
  ...
  it('should throw BabyNotFoundException when baby is missing', async () => {
    ...
    ).rejects.toThrow(BabyNotFoundException);
  ...
  it('should throw ForbiddenBabyAccessException when caller does not own baby', async () => {
    ...
    await expect(
      useCase.execute(mockBabyId, mockEventId, mockOwnerId, {}),
    ).rejects.toThrow(ForbiddenBabyAccessException);
